...
WITH session_applications AS (
    SELECT
        *,
        ROW_NUMBER() OVER (PARTITION BY session_id ORDER BY submitted_at) AS app_rank
    FROM {{ ref('stg_applications') }}
),
single_application AS (
    SELECT * FROM session_applications WHERE app_rank = 1
)
SELECT
  vs.session_id,
  vs.user_id,
  ...
  -- Application fields (now 1:1 with session)
  a.application_id,
  a.service_code,
...
LEFT JOIN {{ ref('stg_users') }} u ON vs.user_id = u.user_id
LEFT JOIN {{ ref('stg_voice_ai_metrics') }} m ON vs.session_id = m.session_id
LEFT JOIN single_application a ON vs.session_id = a.session_id
